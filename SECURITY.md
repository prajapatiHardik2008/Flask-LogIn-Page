...
- Reset tokens are signed, time-limited, and do not disclose whether an email exists.

## Production checklist

- Set a long random `SECRET_KEY` outside source control and use a managed PostgreSQL database.
- Set `SESSION_COOKIE_SECURE=true`, terminate TLS correctly, and configure trusted proxy behavior.
- Replace the development reset-link preview with an email provider. Do not log reset URLs, tokens, passwords, or secrets.
